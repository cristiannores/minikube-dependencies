...
const kafkaProducer = async () => {
    ...
    try {
        ...
        const resSent =  await producer.send({
            topic: 'oms.interceptor.pfo_to_oms',
            messages: [
                {
                    value: `
...
`, key: uuidv4()
        ...
        console.log('error Code', resSent[0].errorCode)
        console.log('offset', resSent[0].offset)
        console.log('partition', resSent[0].partition)
        console.log('timestamp', resSent[0].timestamp)
        console.log('topicName', resSent[0].topicName)
        console.log('Message sent', resSent[0].topicName)
        await producer.disconnect()
    }catch (e) {
        console.log('error sending to kafka',e.message);
    }
